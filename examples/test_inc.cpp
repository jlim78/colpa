...
bool isPointValid(cv::Mat image, const ompl::base::State* state) { //Deprecated
...
void displayChanges(std::string obstacleFile, colpa::datastructures::Graph graph, std::vector<colpa::datastructures::Edge>& changed_edges) {
  ...
  // ====================== Draw Changes ==========================
  for (std::vector<colpa::datastructures::Edge>::iterator ei = changed_edges.begin() ; ei != changed_edges.end(); ++ei)
  {
    colpa::datastructures::Vertex startVertex = source(*ei, graph);
    colpa::datastructures::Vertex endVertex = target(*ei, graph);
...
void displayTree(std::string obstacleFile, int duration, colpa::datastructures::Graph graph) {
  ...
  //     // Check if this belongs to tree
  //     if (graph[endVertex].getParent()==startVertex ||graph[startVertex].getParent()==endVertex  )
  //     // if (graph[endVertex].hasParent() || graph[startVertex].hasParent())
  //     {
  //       lineWidth = 3;
  //     }
  //     else
  ...
  colpa::datastructures::Vertex pathVertex = 1;
  ...
  //=============================  Vertex Drawing ================================//
  colpa::datastructures::VertexIter vi, vi_end;
  ...
  cv::namedWindow("COLPA*",cv::WINDOW_AUTOSIZE);
  cv::moveWindow("COLPA*",100,0);
  // cv::resizeWindow("Lazy COLPA*",200,200);
  cv::imshow("COLPA*", image);
  cv::waitKey(duration);
}
...
int main(int argc, char* argv[]) {
  ...
  displayTree(obstacleLocation1, 0 , planner.getGraph());
  ...
  for (int i = 0; i < totalIteration; i++) {
    ...
    if (status == ompl::base::PlannerStatus::EXACT_SOLUTION) {
      ...
      displayTree(obstacleLocation1, 800 , planner.getGraph());
    }
